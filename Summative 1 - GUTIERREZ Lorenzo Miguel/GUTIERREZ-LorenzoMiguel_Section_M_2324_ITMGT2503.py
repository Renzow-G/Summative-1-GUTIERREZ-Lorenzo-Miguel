...
#### Remove all the unnecessary punctuation symbols denoted in the string below by converting it to a space
...
for line in jsondata.items():
    new_dict[count] = str(line[1].upper())
    newer_dict[count] = new_dict[count]
    for tag in tags:
        newer_dict[count] = newer_dict[count].replace(tag,"")
    clean_dict[count] = newer_dict[count]
    for punctuation in unnecessary_punctuation:
        clean_dict[count] = clean_dict[count].replace(punctuation," ")
...
for line in clean_dict.values():
    words = line.split()
    for word in words:
        if len(word) < 3:
            continue
        if word not in wordcount_dictionary:
            wordcount_dictionary.update({word:0})
        value = wordcount_dictionary.get(word) + 1
...
# assuming not allowed to use solvers such as SciPY since topics restricted to until loops
...
# while Gold >= 750 and Lumber >= 300 and Food >= 75:
#     Gold -= 750
#     Lumber -= 300
#     Food -= 75
#     optimal_C += 1

#     if Gold >= 600 and Lumber >= 750 and Food >= 45:
#         Gold -= 600
#         Lumber -= 750
#         Food -= 45
#         optimal_R += 1
    
#         if Gold >= 1000 and Lumber >= 500 and Food >= 50:
#             Gold -= 1000
#             Lumber -=500
#             Food -= 50
#             optimal_F += 1

# maybe tests all possible values of each decision variable using range or some other iteration? get max from those?

for item in range(0,optimal_F):
    Gold -= 1000
    Lumber -= 500
    Food -= 50

for item in range(0,optimal_R):
    Gold -= 600
    Lumber -= 750
    Food -= 45

for item in range(0,optimal_C):
    Gold -= 750
    Lumber -= 300
    Food -= 75
...
print(optimal_Z,optimal_F,optimal_R,optimal_C)
print(Gold,Lumber,Food)
...
def Collatz_winner(number_list):
    ...
    for number in number_list:
        master_dict[number] = Collatz(number)
    for item in master_dict.values():
        lengths_list.append(item.get('collatz_length'))
        max_list.append(item.get('max_collatz'))
    
    # length_index = lengths_list.index(max(lengths_list))

    max_length = max(lengths_list)
    length_index_set = [index for index in range(len(lengths_list)) if lengths_list[index] == max_length]
    
    # max_index = max_list.index(max(max_list))

    max_max = max(max_list)
    max_index_set = [index for index in range(len(max_list)) if max_list[index] == max_max]
    
    keys_list = list(master_dict.keys())
    # print(type(max_index_set))

    def intersection(lst1, lst2):
        lst3 = [value for value in lst1 if value in lst2]
        return lst3

    intersection_list = intersection(length_index_set, max_index_set)
    
    if len(intersection_list) >= 1:
        # print(length_index,max_index)
        # print(lengths_list)
        # print(max_list)
        return keys_list[intersection_list[0]]
    else:
        # print(length_index,max_index)
        # print(lengths_list)
        # print(max_list)
...
# problem with max values that are in multiple indices, limitation of index function (resolved)
